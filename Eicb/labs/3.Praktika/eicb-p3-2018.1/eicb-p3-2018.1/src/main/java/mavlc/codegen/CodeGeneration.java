...
public class CodeGeneration extends ASTNodeBaseVisitor<Instruction, Integer> {
	...
	public Instruction visitVariableDeclaration(VariableDeclaration variableDeclaration, Integer arg1) {
		...
		System.out.println("VisitvariDecl wird aufgerufen");
		return null;
	}
	...
	public Instruction visitVariableAssignment(VariableAssignment variableAssignment, Integer arg1) {
		//TDO Task 3.2
		System.out.println("VisitvarAssign wird aufgerufen");
		variableAssignment.getValue().accept(this, null);
		variableAssignment.getIdentifier().accept(this, null);
		assembler.storeToStackAddress(variableAssignment.getValue().getType().wordSize());
		return null;
	...
	public Instruction visitLeftHandIdentifier(LeftHandIdentifier leftHandIdentifier, Integer arg1) {
		//TDO Task 3.2
		System.out.println("VisitLHIdent wird aufgerufen");
	...
	public Instruction visitMatrixLHSIdentifier(MatrixLHSIdentifier matrixLHSIdentifier, Integer arg1) {
		//TDO Task 3.2
		System.out.println("VisitMatLHIdent wird aufgerufen");
		int offset = matrixLHSIdentifier.getDeclaration().getLocalBaseOffset();
		assembler.loadAddress(Register.LB, offset);
		MatrixType m =(MatrixType) matrixLHSIdentifier.getDeclaration().getType();
		matrixLHSIdentifier.getXIndex().accept(this, null);
		assembler.loadIntegerValue(m.getxDimension());
		assembler.emitIntegerMultiplication();
		matrixLHSIdentifier.getYIndex().accept(this, null);
		assembler.loadIntegerValue(m.getyDimension());
		assembler.emitIntegerMultiplication();
		//assembler.storeToStackAddress(m.wordSize());
	...
	public Instruction visitRecordLHSIdentifier(RecordLHSIdentifier recordLHSIdentifier, Integer arg1) {
		//TDO Task 3.2System.out.println("VisitMatLHIdent wird aufgerufen");
		 String modVal = recordLHSIdentifier.getElementName();
		 RecordType rt = (RecordType) recordLHSIdentifier.getDeclaration().getType();
			                   RecordElementDeclaration red = rt.getTypeDeclaration().getElement(modVal);
			                   red.accept(this, null);
			                   return null;
	...
	public Instruction visitForEachLoop(ForEachLoop forEachLoop, Integer arg1) {
		...
		Type type;
		if(forEachLoop.getStructExpr().getType() instanceof VectorType) 
			type = ((VectorType) forEachLoop.getStructExpr().getType()).getElementType();
		else
			type = ((MatrixType) forEachLoop.getStructExpr().getType()).getElementType();
		
		assembler.addDeclaredEntity(forEachLoop.getIteratorDeclaration());
		assembler.emitPush(forEachLoop.getIteratorDeclaration().getType().wordSize());
		int p = assembler.getNextOffset();
		assembler.loadIntegerValue(0);
		assembler.setNextOffset(p+1);
		int start = assembler.getNextOffset();
		
		forEachLoop.getStructExpr().accept(this, null);
		assembler.resetNextOffset(start+forEachLoop.getStructExpr().getType().wordSize());

		int loopstart = assembler.getNextInstructionAddress();
		assembler.loadValue(Register.LB, 1, p);
		
		assembler.loadAddress(Register.LB, start);
		assembler.emitIntegerAddition();
		assembler.loadFromStackAddress(type.wordSize());
		assembler.storeValue(Register.LB, type.wordSize(), forEachLoop.getIteratorDeclaration().getLocalBaseOffset());
		forEachLoop.getLoopBody().accept(this, null);
		
		if(forEachLoop.getIteratorDeclaration().isVariable()) {
			IdentifierReference help = (IdentifierReference) forEachLoop.getStructExpr();
			int offset2 = help.getDeclaration().getLocalBaseOffset();
			assembler.loadLocalValue(forEachLoop.getIteratorDeclaration().getType().wordSize(), forEachLoop.getIteratorDeclaration().getLocalBaseOffset());
			assembler.loadLocalValue(1, p);
			assembler.loadAddress(Register.LB, offset2);
			assembler.emitIntegerAddition();
			assembler.storeToStackAddress(type.wordSize());
		
		}
		assembler.loadLocalValue(1, p);
		assembler.loadIntegerValue(type.wordSize());
		assembler.emitIntegerAddition();
		assembler.storeLocalValue(1, p);
		
		assembler.loadLocalValue(1, p);
		assembler.loadIntegerValue(forEachLoop.getStructExpr().getType().wordSize());
		...
		iterator.accept(this, null);
		Expression struct = forEachLoop.getStructExpr();
	//	iterator.accept(this, null);
		int offset = iterator.getLocalBaseOffset();
		//foreach: der Iterator vewrkrpert ein Elemen
...
//		}
	...
	public Instruction visitSwitchStatement(SwitchStatement switchCaseStatement, Integer arg1) {
		...
		Instruction prevjump = null;
		List<Instruction> l = new ArrayList<>();
		Instruction hilf = null;
		for(Case c: switchCaseStatement.getCases()) {
			int addr = assembler.getNextInstructionAddress();
			//wir sollen hier den richtigen Fall reincoden
			//wir brauchen Sprung Adressen um von case zu case zu gehen
			assembler.loadValue(Register.ST, Type.getIntType().wordSize(), -1);
			assembler.loadIntegerValue(c.getCondition());
			assembler.emitIntegerComparison(Comparison.EQUAL);
			Instruction nextjump = assembler.emitConditionalJump(false, -1);
			if(prevjump != null) {
				assembler.backPatchJump(prevjump, addr);
			}
			prevjump = nextjump;
			
			this.visitCase(c, arg1);
			
			Instruction jump = assembler.emitJump(-1);
			l.add(jump);
			
			
			
		}
		if(!switchCaseStatement.getDefaultCases().isEmpty()) {
			// Es git einen Daefault Fall. dieser muss auch durchlaufen werden, falls alle anderen Faelle nicht klappen
			this.visitDefault(switchCaseStatement.getDefaultCases().get(0), arg1);
			
		}
		
		int end = assembler.getNextInstructionAddress();
		for(Instruction e : l) 
				assembler.backPatchJump(e, end);
		return null;
		//throw new UnsupportedOperationException();
	}
	...
	public Instruction visitDivision(Division division, Integer arg1) {
		...
		Type leftType = division.getLeftOp().getType();
		Type rightType = division.getRightOp().getType();
		if(resultType.equals(IntType.getIntType())){
	...
	public Instruction visitRecordElementSelect(RecordElementSelect recordElementSelect, Integer arg1) {
		//TOO Task 3.3
		Expression struct = recordElementSelect.getRecord();
		struct.accept(this, null);
		
		assembler.loadAddress(Register.ST, struct.getType().wordSize()*-1);
		int offsetrec = 0;
		
		for(RecordElementDeclaration r: ((RecordType) recordElementSelect.getRecord().getType()).getTypeDeclaration().getElements()) {
			
		if(r.getName().equals(recordElementSelect.getElementName())) {
			break;
		}
		offsetrec += r.getType().wordSize();
		}
		assembler.loadIntegerValue(offsetrec);
		assembler.emitIntegerAddition();
		assembler.loadFromStackAddress(recordElementSelect.getType().wordSize());
		assembler.emitPop(recordElementSelect.getType().wordSize(),recordElementSelect.getRecord().getType().wordSize());
	...
	public Instruction visitSelectExpression(SelectExpression exp, Integer arg1){
		//TOO Task 3.4.3 UNTERSCHIED ZU IF ist, dass die true/false cases hier expressions und nicht wie bei if statements sind. Sprung Befehle mssena nders gehandhabt werden
