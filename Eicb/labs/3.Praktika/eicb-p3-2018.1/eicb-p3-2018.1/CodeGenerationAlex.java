...
public class CodeGeneration extends ASTNodeBaseVisitor<Instruction, Integer> {
	...
	public Instruction visitLeftHandIdentifier(LeftHandIdentifier leftHandIdentifier, Integer arg1) {
		//TODO Task 3.2
		int offsetAddr = leftHandIdentifier.getDeclaration().getLocalBaseOffset();
		assembler.loadAddress(Register.LB, offsetAddr);
		return null;
	}
	...
	public Instruction visitMatrixLHSIdentifier(MatrixLHSIdentifier matrixLHSIdentifier, Integer arg1) {
		//TODO Task 3.2
		int offsetAddr = matrixLHSIdentifier.getDeclaration().getLocalBaseOffset();
		assembler.loadAddress(Register.LB, offsetAddr);
		MatrixType mt = (MatrixType) matrixLHSIdentifier.getDeclaration().getType();
		matrixLHSIdentifier.getYIndex().accept(this, null);
		assembler.loadIntegerValue(mt.getyDimension());
	...
	public Instruction visitVectorLHSIdentifier(VectorLHSIdentifier vectorLHSIdentifier, Integer arg1) {
		//TODO Task 3.2
		int offsetAddr = vectorLHSIdentifier.getDeclaration().getLocalBaseOffset();
	...
	public Instruction visitForEachLoop(ForEachLoop forEachLoop, Integer arg1) {
		//TODO Task 3.4.2
		forEachLoop.getIteratorDeclaration().accept(this, null);
		int itDecOffset = forEachLoop.getIteratorDeclaration().getLocalBaseOffset();
		
		int compAddr = assembler.getNextInstructionAddress();
		//TODO ITERATOR
		
		// Wenn Schleife zu Ende ist -> Jump
		Instruction endJump = assembler.emitConditionalJump(false, -1);
		// Anfang Schleifen Body
		forEachLoop.getLoopBody().accept(this, null);
		assembler.emitJump(compAddr);
		// Ende der Schleife
		int endAddr = assembler.getNextInstructionAddress();
		assembler.backPatchJump(endJump, endAddr);
		return null;
	}
...
//		}
	...
	public Instruction visitSwitchStatement(SwitchStatement switchCaseStatement, Integer arg1) {
		...
		List<Instruction> endList = new ArrayList<>();
		
		Instruction previousJumpInstruction = null;
		
		for(Case switchCase : switchCaseStatement.getCases()) {
			int addr = assembler.getNextInstructionAddress();
			
			assembler.loadValue(Register.ST, Type.getIntType().wordSize(), -1);
			assembler.loadIntegerValue(switchCase.getCondition());
			assembler.emitIntegerComparison(Comparison.EQUAL);
			Instruction jumpNextCase = assembler.emitConditionalJump(false, -1);
			
			if(previousJumpInstruction != null) {
				assembler.backPatchJump(previousJumpInstruction, addr);
			}
			previousJumpInstruction = jumpNextCase;
			
			switchCase.accept(this, null);
			Instruction jump = assembler.emitJump(-1);
			endList.add(jump);
		}

		if(!switchCaseStatement.getDefaultCases().isEmpty()) {
			Default def = switchCaseStatement.getDefaultCases().get(0);
			def.accept(this, null);
		}
		
		int endAddr = assembler.getNextInstructionAddress();
		for(Instruction end : endList) {
			assembler.backPatchJump(end, endAddr);
		}
		return null;
	}
	...
	public Instruction visitRecordElementSelect(RecordElementSelect recordElementSelect, Integer arg1) {
		...
		for(RecordElementDeclaration Relement :((RecordType) recordElementSelect.getRecord().getType()).getTypeDeclaration().getElements()) {
			if(Relement.getName().equals(recordElementSelect.getElementName())) {
				break;
			}
			offset += Relement.getType().wordSize();
		}
